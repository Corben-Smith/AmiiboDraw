...
public class ConsoleImage
{
     ...
     * 
     */
    public static void DrawImage(Bitmap bmp, int xSkip = 1, int ySkip = 2, int xBuffer = 0, int yBuffer = 0, string title = "Console", bool auto = false)
    {
        ...
        if (auto)
        {
            ySkip = FConsole.WindowHeight/20;
            xSkip = FConsole.WindowWidth/20;
        }
        ...
        for (short y = 0; y < bmp.Height; y += (short)ySkip)
        {
            ...
            for (short x = 0; x < bmp.Width; x += (short)xSkip)
            {
                ...
                //char dispLetter = CalculateLuminance(bmp.GetPixel(x,y));
    ...
    static char SetCharacter(Bitmap bmp, short x, short y, int xSkip, int ySkip)
    {
        ...
        {
            if (bottom.A == 0 && left.A == 0)
            {
                return '╲';
            }
            else if (bottom.A == 0 && right.A == 0)
            {
                return '╱';
            }
            else
